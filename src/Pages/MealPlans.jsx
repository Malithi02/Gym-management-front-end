...
const MealPlans = () => {
    ...
    const [loading,setLoading] = useState(false);
     useEffect(() =>{
        setLoading(true);
        axios
        .get('http://localhost:3000/trainer-dashboard/mealplans')
        .then((response) => {
            ...
            <table className='w-full border-separate border-spacing-2'>
             ...
             <tbody>
                {mealplans.map((mealplan,index) => (
                    <tr key={mealplan.id} className ='h-8'>
                        ...
                        <td className='flex justify-center gap-x-4'>
                            <Link to={`/mealplans/details/${mealplan.id}`}><BsInfoCircle className='text-green-800'/></Link>
                            <Link to={`/mealplans/edit/${mealplan.id}`}><AiOutlineEdit className='text-yellow-600'/></Link>
                            <Link to={`/mealplans/delete/${mealplan.id}`}><MdOutlineDelete className='text-red-600'/></Link>
                        </td>
                    </tr>
                ))}
             </tbody>
